...
def TemaOscuro(*args):
    ...
    estilos_label1.configure('Label1.TLabel', background="#010924", foreground="white", borderwidth=0)
    estilos_label2.configure('Label2.TLabel', background="#010924", foreground="white", borderwidth=0)

    estilos_botones_numeros.configure('Botones_numeros.TButton', background="#FF8000", foreground="white")
...
def TemaClaro(*args):
    ...
    estilos_label1.configure('Label1.TLabel1',background="#DBDBDB", foreground="black" )
    estilos_label2.configure('Label2.TLabel2',background="#DBDBDB", foreground="black" )
...
def tantoporciento():
    try:
        # Obtenemos el valor en la entrada actual (entrada2)
        valor_actual = float(entrada2.get())

        # Si ya hay una operación en curso (entrada1), aplicamos el porcentaje
        if entrada1.get():
            operacion = entrada1.get()
            # El último número antes del operador será el valor base
            valor_base = float(operacion[:-1])  # Quitamos el último carácter que será el operador

            # Calculamos el porcentaje del valor base
            resultado = valor_base * valor_actual / 100

            # Actualizamos la pantalla con el resultado del porcentaje
            entrada2.set(str(resultado))
        else:
            # Si no hay una operación previa, simplemente lo tratamos como un porcentaje aislado
            entrada2.set(str(valor_actual / 100))
    except ValueError:
        # Si hay un error (como un valor no válido), limpiamos la entrada
        entrada2.set("vete a la escuela")
...
estilos_label1.configure('Label1.TLabel', font="arial 15", anchor="e", borderwidth=0)

estilos_label2 = ttk.Style()
estilos_label2.configure('Label2.TLabel', font="arial 22", anchor="e", borderwidth=0)
...
button_tantoporciento = ttk.Button(mainframe,text="%", style='Botones_restantes.TButton', command=lambda: tantoporciento ())
...
button_tantoporciento.grid(column=2, row=7,  sticky=(W,N,E,S))


for child in mainframe.winfo_children():
    child.grid_configure(ipady=10, padx=1, pady=1 )

root.bind('<KeyPress-m>', TemaOscuro)  
root.bind('<KeyPress-n>', TemaClaro)
root.bind('<Key>', ingresarValoresTeclado)
root.bind("<KeyPress-b>", borrar)
root.bind("<KeyPress-v>", borrarTodo)
